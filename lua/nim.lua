...
local function report_line(line)
	...
	if line:find("Warning") == nil and line:find("Error") == nil and line:find("Hint") == nil then
		return
	end
...
local function check_buffer()
	buffers = {}
	vim.fn.jobstart({ "nim", "check", vim.api.nvim_buf_get_name(0) }, {
		...
		on_exit = function()
			...
			local list = {}
			for i = 1, #diagnostics.Warning do
				table.insert(list, diagnostics.Warning[i])
			end
			for i = 1, #diagnostics.Error do
				table.insert(list, diagnostics.Error[i])
			end
			for i = 1, #diagnostics.Hint do
				table.insert(list, diagnostics.Hint[i])
			end
